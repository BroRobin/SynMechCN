%% Analyse eyelid behavior during in vivo patch recordings
...
for jj=1
    ...
    %% Calculate statistics CS
    stats_CSelectrophysiology;

    %% Calculate statistics US
    ...
    %% Finish up
    % close all;
    % clearvars -except Param Eyeblink_signal R S jj
...
% Spkresults.traceBorders = [0 732.2035];
% Spkresults.deltaV = 20; % Define the amount of V between each trace
% Spont.spontBorders = [208.8802];
% Spont.ylim = [-50 0];
% Spont.plotLength= 5; % in sec
% OnsetAnalysis.ephysType = 1; % 1 = IPSP, 2 = EPSP
% Nunfinishedtrials = 6;     % How  unfinished trials that need to be removed (by stopping the protocol in the middle)?