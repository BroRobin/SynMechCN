...
% Detect CRs/EOs based on values that exceed 10% of the UR. Calculate parameters of CR and UR eyelid movements.
...
% Detect CR's and eyelid movements. 10% of UR is considered to be a CR.
for curr_trial = 1: size(Eyeblink_signal.GoTr_TimeStamps, 2)
    if Eyeblink_signal.GoTr_IDstim(curr_trial)==3 % Paired
        ...
        if (max(Eyeblink_signal.outlierfree_paired(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) > ((Param.CRthreshold_UR*R.trial_USonly_avgPeak)+R.trial_baseline_vals(1, curr_trial))) && (min(Eyeblink_signal.outlierfree_paired(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) < (R.trial_baseline_vals(1, curr_trial)-(Param.CRthreshold_UR*R.trial_USonly_avgPeak)))
        ...
        elseif max(Eyeblink_signal.outlierfree_paired(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) > ((Param.CRthreshold_UR*R.trial_USonly_avgPeak)+R.trial_baseline_vals(1, curr_trial))
            R.trial_success_flags(1,curr_trial)=1;
        elseif min(Eyeblink_signal.outlierfree_paired(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) < (R.trial_baseline_vals(1, curr_trial)-(Param.CRthreshold_UR*R.trial_USonly_avgPeak))
            R.trial_success_flags(1,curr_trial)=2;
        end

    elseif Eyeblink_signal.GoTr_IDstim(curr_trial)==1 % CSonly
        if (max(Eyeblink_signal.outlierfree_CSonly(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) > ((Param.CRthreshold_UR*R.trial_USonly_avgPeak)+R.trial_baseline_vals(1, curr_trial))) && (min(Eyeblink_signal.outlierfree_CSonly(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) < (R.trial_baseline_vals(1, curr_trial)-(Param.CRthreshold_UR*R.trial_USonly_avgPeak)))
            % In case you have both CR and opening: the one with the highest amplitude counts
        ...
        elseif max(Eyeblink_signal.outlierfree_CSonly(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) > ((Param.CRthreshold_UR*R.trial_USonly_avgPeak)+R.trial_baseline_vals(1, curr_trial))
            R.trial_success_flags(1,curr_trial)=1;
        elseif min(Eyeblink_signal.outlierfree_CSonly(curr_trial,round(.55/Param.Ts):round(.75/Param.Ts))) < (R.trial_baseline_vals(1, curr_trial)-(Param.CRthreshold_UR*R.trial_USonly_avgPeak))
            R.trial_success_flags(1,curr_trial)=2;
        end
    else
...
for curr_trial = 1: size(Eyeblink_signal.GoTr_TimeStamps, 2)
    ...
    if Eyeblink_signal.outlierindx_paired(curr_trial)==1 && R.trial_success_flags(1,curr_trial)==1 && Eyeblink_signal.GoTr_IDstim(1,curr_trial)==3; % Paired CR
        ...
        index= (Eyeblink_signal.outlierfree_paired(curr_trial,1:round(.75/Param.Ts)) > (R.trial_baseline_vals(1,curr_trial)+(Param.CRthreshold_UR*R.trial_USonly_avgPeak)));
    ...
    elseif Eyeblink_signal.outlierindx_CSonly(curr_trial)==1 && R.trial_success_flags(1,curr_trial)==1 && Eyeblink_signal.GoTr_IDstim(1,curr_trial)==1; % CSonly CRs
        ...
        index= (Eyeblink_signal.outlierfree_CSonly(curr_trial,1:round(.75/Param.Ts)) > (R.trial_baseline_vals(1,curr_trial)+(Param.CRthreshold_UR*R.trial_USonly_avgPeak)));
        index(1:round(.51/Param.Ts))=0; % from 510 ms onwards
        %         index=index(1:I);
        onset_idx = find(index==1,1,'last');
        R.trial_CRs(curr_trial,3) = Param.tvalues(find(index(1:onset_idx)==0,1,'last'));
