...
%% 500ms before and after

xtimes_behav = [-0.5:(1/Param.fs):0.5];
xtimes_Vm = [-0.5:(1/100000):0.5];
...
try
    for ii=1:size(Eyeblink_signal.ind_trials,1)
        ...
        idx_Vm=[];
        if CS.resp==1
            if isnan(PSP_onset.onset_EPSP_CSperiod(ii,1))==0 && isnan(behav.onset_SEO(ii,1))==0
                idx = round(((behav.onset_SEO(ii,1)+500)*10^-3)*Param.fs);
                ...
                idx = round(((behav.onset_SEO(ii,1)+500)*10^-3)*100000);
        ...
        elseif CS.resp==2
            if isnan(PSP_onset.onset_IPSP_CSperiod(ii,1))==0 && isnan(behav.onset_SEO(ii,1))==0
                idx = round(((behav.onset_SEO(ii,1)+500)*10^-3)*Param.fs);
                Onsetnorm.eyelidOnset_eyelid_trials(ii,:)=Eyeblink_signal.ind_trials(ii,idx-(0.5*Param.fs):idx+(0.5*Param.fs));
                Onsetnorm.eyelidOnset_eyelid_trials(ii,:)=Onsetnorm.eyelidOnset_eyelid_trials(ii,:)-nanmean(Onsetnorm.eyelidOnset_eyelid_trials(ii,1:0.25*Param.fs));
                idx = round(((behav.onset_SEO(ii,1)+500)*10^-3)*100000);
