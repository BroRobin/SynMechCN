%% Analyze eyelid movements evoked by Blue light opto stimulation
...
for kk=1:12
    ...
    if kk==1
        cd('[path]')
        load('DataOutput_2609181_Block-1.mat')
        savename = 'Analysis_2609181_Block-1.mat'
    elseif kk==2
        cd('[path]')
        load('DataOutput_2609182_Block-2.mat')
        savename = 'Analysis_2609182_Block-2.mat'
    elseif kk==3
        cd('[path]')
        load('DataOutput_2609183_Block-1.mat')
        savename = 'Analysis_2609183_Block-1.mat'
    elseif kk==4
        cd('[path]')
        load('DataOutput_2609184_Block-2.mat')
        savename = 'Analysis_2609184_Block-2.mat'
    elseif kk==5
        %% D5
        cd('[path]')
        load('DataOutput_2609181_Block-4.mat')
        savename = 'Analysis_2609181_Block-4.mat'
    elseif kk==6
        cd('[path]')
        load('DataOutput_2609182_Block-4.mat')
        savename = 'Analysis_2609182_Block-4.mat'
    elseif kk==7
        cd('[path]')
        load('DataOutput_2609183_Block-5.mat')
        savename = 'Analysis_2609183_Block-5.mat'
    elseif kk==8
        cd('[path]')
        load('DataOutput_2609184_Block-4.mat')
        savename = 'Analysis_2609184_Block-4.mat'
    elseif kk==9
        %% D10
        cd('[path]')
        load('DataOutput_2609181_Block-6 copy.mat')
        savename = 'Analysis_2609181_Block-6 copy.mat'
    elseif kk==10
        cd('[path]')
        load('DataOutput_2609181_Block-8 copy.mat')
        savename = 'Analysis_2609181_Block-8 copy.mat'
    elseif kk==11
        cd('[path]')
        load('DataOutput_2609183_Block-12 copy.mat')
        savename = 'Analysis_2609183_Block-12 copy.mat'
    elseif kk==12
        cd('[path]')
        load('DataOutput_2609184_Block-6 copy.mat')
        savename = 'Analysis_2609184_Block-6 copy.mat'
    end

    %% Process the data
    for ii=1:size(signal.outlierfree_optoOnly,1) %normalize opto only to 50ms before the baseline value (250-500ms after start trial)
        opto.outlierfree_optoOnly_norm(ii,:) = signal.outlierfree_optoOnly(ii,:)-nanmean(signal.outlierfree_optoOnly(ii,round(0.45*P.fs_mdmt):round(0.5*P.fs_mdmt)));

        % exclude traces where baseline > 5*SD of baseline
        if sum(opto.outlierfree_optoOnly_norm(ii,1:round(0.5*P.fs_mdmt))>(5*nanstd(opto.outlierfree_optoOnly_norm(ii,round(0.45*P.fs_mdmt):round(0.5*P.fs_mdmt)))))>0
    ...
    ind_trials_amp = [ind_trials_amp; opto.amp(isnan(opto.amp)==0)'; NaN];
    ind_trials_AUC = [ind_trials_AUC; opto.AUC(isnan(opto.AUC)==0)'; NaN];
...
opto.D0_traces_sem = nanstd(opto.D0_traces,1)/sqrt(4);
opto.D5_traces_sem = nanstd(opto.D5_traces,1)/sqrt(4);
opto.D10_traces_sem = nanstd(opto.D10_traces,1)/sqrt(4);
...
% ylim([-2 3]);
% xlim([-0.2 0.5]);
% yticks([-2 -1 0 1 2 3]); % MODIFY TO FIT FIGURES
...
%% Save figures
cd('[path]')
figname = 'Opto_Blue_averages.fig';
saveas(gcf,figname,'fig');
figname = 'Opto_Blue_averages.pdf';
